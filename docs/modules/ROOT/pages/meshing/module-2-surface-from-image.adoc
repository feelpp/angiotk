...
The resulting surface can be sudivided using 3 methods: *linear* (original points are preserved and linear subdivisions are created in between), *butterfly* (original points are preserved and new points are added) or *loop* (original points are displaced and new points are added).
