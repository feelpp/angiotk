...
:m_root(0),m_img(img)
{
    FlatSE connexity;
	connexity.make2DN8();
	SalembierRecursiveImplementation<T> strategy(this,gt,connexity);
...
template <class T>
int ComponentTree<T>::meanFiltering(int tMin, int tMax)
{
	if(m_root!=0)
		{
        std::queue <Node *> fifo;
		...
		while(!fifo.empty() )
			...
			if(curNode->mean < tMin  || curNode->mean > tMax)
				{
				curNode->active=false;
				}
...
{
	Node *res=indexNodes(offset);
	return res;
}


template <class T>
Image <U8> ComponentTree<T>::coordToComponent(TCoord x, TCoord y)
{
    TOffset offset=m_img.getOffset(x,y);
	return offsetToComponent(offset);
}

template <class T>
Image <U8> ComponentTree<T>::offsetToComponent(TOffset offset)
{

    Node *n=offsetToNode(offset);
    if(n!=0)
        {
        std::vector<TOffset> offsetComponent=merge_pixels(n);

        Image <U8> imageComponent(m_img.getSizeX(),m_img.getSizeY());
        imageComponent.fill(0);

        for(int i=0; i<offsetComponent.size(); i++)
            {
            Point<TCoord> p=m_img.getCoord(offsetComponent[i]);
            imageComponent(p.x,p.y)=255;
            }
...
// BN 31/12/08
// compute grey-level mean for each component
// to be computer AFTER component area
template <class T>
double SalembierRecursiveImplementation<T>::computeMean(Node *tree)
{
	if(tree!=0)
		{
		double sum=tree->h*tree->pixels.size();
...
// BN 04/01/09
// compute grey-level variance for each component
// to be computer AFTER area and mean attributes
//template <class T>
//double SalembierRecursiveImplementation<T>::computeVar(Node *tree)
//{
//	if(tree!=0)
//		{
//		double sum=(tree->h-tree->mean)*(tree->h-tree->mean)*tree->pixels.size();
//
//        Node::ContainerChilds::iterator it;
//        for(it=tree->childs.begin(); it!=tree->childs.end(); ++it)
//            {
	...
	if(tree!=0)
		...
		while(!fifo.empty() )
			...
			if(n->contourLength!=0)
				{
				n->compacity=(400.0*M_PI*n->area)/(n->contourLength*n->contourLength);
				//WARNING!!!!!!!!!!!!!!
// 				if(n->compacity>100)
// 					n->compacity=100;
				}
...
int SalembierRecursiveImplementation<T>::computeAttributes(Node *tree)
{
	if(tree!=0)
		...
		// BN 31/12/08
        //tree->mean=computeMean(tree)/tree->area;
...
inline int SalembierRecursiveImplementation<T>::update_attributes(Node *n, TOffset &imBorderOffset)
{
    ...
    //R-signature:
    //Chaque point rajoute une sinusoide dans l'espace de Radon associ� au noeud
    //il est donc n�cessaire:
    // 1) de m�moriser pour chaque noeud l'image contenant l'espace de Radon: couteux en m�moire!!!
    // 2) d'effectuer un post-traitement afin d'additionner les espaces de Radon des noeuds fils
    // 3) de calculer la R-signature pour chaque noeud
    return 1;
}
...
//	if(imCoord.y > n->ymax) n->ymax=imCoord.y;
    ...
    // update attributes related to p* and n* (see paper)
    if(gt(imCoord)!=0)
        {
        n->ps++;
        }
...
inline int SalembierRecursiveImplementation<T>::flood(int h)
{
	...
	while(!hq[h].empty())
		...
		hq[h].pop();
		//std::cout << "Point traité: " << p << " " << (int)imBorder(p) << "\n";
		...
		if(index[h][STATUS(p)]==0)
			...
			//std::cout << "Création du noeud C("<< h << "," << STATUS[p] << ")\n";
			}
...
Node * SalembierRecursiveImplementation<T>::computeTree()
{
	...
	//std::cout << "Initialisation terminée Hmin= " << (int)hMin << "\n";

	this->flood(hToIndex(hMin));

	//std::cout << "Calcul terminé Hmin= " << (int)hMin << "\n";
...
//initialize global index for nodes
template <class T>
int SalembierRecursiveImplementation<T>::init(Image <T> &img, Image<U8> &, FlatSE &connexity)
{
