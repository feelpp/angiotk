...
vtkStandardNewMacro(angiotkPolyBallModeller);

angiotkPolyBallModeller::angiotkPolyBallModeller()
{
...
int angiotkPolyBallModeller::RequestInformation (
  ...
  vtkInformationVector *outputVector)
{
  ...
  std::cout  << "Sample Dimensions: (" << this->SampleDimensions[0] << ", "
     << this->SampleDimensions[1] << ", "
     << this->SampleDimensions[2] << ")\n";
  std::cout  << "ModelBounds: \n";
  std::cout  << "  Xmin,Xmax: (" << this->ModelBounds[0] << ", " << this->ModelBounds[1] << ")\n";
  std::cout  << "  Ymin,Ymax: (" << this->ModelBounds[2] << ", " << this->ModelBounds[3] << ")\n";
  std::cout  << "  Zmin,Zmax: (" << this->ModelBounds[4] << ", " << this->ModelBounds[5] << ")\n";

  return 1;
}

int angiotkPolyBallModeller::FillInputPortInformation(int, vtkInformation *info)
{
  info->Set(vtkAlgorithm::INPUT_REQUIRED_DATA_TYPE(), "vtkPolyData");
  return 1;
}

int angiotkPolyBallModeller::RequestData(
  vtkInformation *vtkNotUsed(request),
  ...
  vtkInformationVector *outputVector)
{
  ...
  if (!this->UsePolyBallLine)
    ...
    angiotkPolyBallLine* polyBallLine = angiotkPolyBallLine::New();
    ...
    for (i=0; i<numberOfOutputPoints; i++)
      {
	if ( (i % 1000000) == 0)
	std::cout << "evaluate point "<<i<< "/"<<numberOfOutputPoints<<"\n";
      output->GetPoint(i,point);
...
void angiotkPolyBallModeller::PrintSelf(ostream& os, vtkIndent indent)
{
