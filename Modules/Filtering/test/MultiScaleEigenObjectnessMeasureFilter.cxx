...
#include <stdlib.h>
#include <stdio.h>
#include <math.h>		//numerical recipes...
...
int EigenToObjectnessMeasure2D(string file, string fe, string path, float smin, float smax, int ns, int gamma, float Tmin, float Tmax, bool SO)
{
	string  img = file;
	string	 folder = path;
	string ext = fe;
	...
	std::ostringstream onSteps;
    std::ostringstream tMin;
    std::ostringstream tMax;
	osMin<<sMin;
	osMax<<sMax;
	onSteps<<nSteps;
...
// //         input->SetInput(imageReader->GetOuput())
//         calculator -> SetImage(imageReader->GetOuput());
//         calculator -> Compute( );
//         //InputImageType::PixelType minIntensity = calculator -> GetMinimum( );
//         InputPixelType maxIntensity = calculator -> GetMaximum( );
    std::cout<<itk::NumericTraits<InputPixelType>::max()<<std::endl;
...
//         CastToRealFilterType::Pointer caster = CastToRealFilterType::New();
//         caster->SetInput(thresholdResult);
    if(Tmin && Tmax)
    {
...
int EigenToObjectnessMeasure3D(string file, string fe, string path, float smin, float smax, int ns, int gamma, float Tmin, float Tmax, bool SO)
{
	string  img = file;
	string	 folder = path;
	string ext = fe;
...
// 	OutputImageType::Pointer  magnitudeResult = multiScaleEnhancementFilter->GetMagnitudeOutput();
    OutputImageType::Pointer  thresholdResult = multiScaleEnhancementFilter->GetHystheresisThresholdOutput();
