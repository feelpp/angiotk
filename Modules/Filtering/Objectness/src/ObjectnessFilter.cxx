...
int EigenToObjectnessMeasure2D(std::string imageFilename, std::string outputImageFilename, float sMin, float sMax, int nSteps, int gamma, float tMin, float tMax, bool ScaleObj)
{
  bool objBright = true;
  ...
  ewriter->SetFileName(outputImageFilename.c_str());
...
int EigenToObjectnessMeasure3D(std::string imageFilename, std::string outputImageFilename, float sMin, float sMax, int nSteps, int gamma, float tMin, float tMax, bool ScaleObj)
{
  // Define the dimension of the images
...
int main(int argc, char* argv[])
{
  ...
  std::string imgOutput = argv[2]; 
  int dimension = atoi(argv[3]);
  float sMin = atof(argv[4]);
  float sMax = atof(argv[5]);
  int nSteps = atoi(argv[6]);
  int gamma = atoi(argv[7]);
  float Tmin, Tmax;     
  bool SO=false;
  ...
  if(dimension == 2)
    {
    r = EigenToObjectnessMeasure2D(img, imgOutput, sMin, sMax, nSteps, gamma, Tmin, Tmax, SO);
    }
  else if(dimension == 3)
    {
    r = EigenToObjectnessMeasure3D(img, imgOutput, sMin, sMax, nSteps, gamma, Tmin, Tmax, SO);
    }
  else
    {
    return EXIT_FAILURE;
