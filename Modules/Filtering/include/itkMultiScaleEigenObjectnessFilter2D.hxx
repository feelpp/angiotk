...
=========================================================================*/
#ifndef __itkMultiScaleEigenObjectnessFilter2D_hxx
#define __itkMultiScaleEigenObjectnessFilter2D_hxx
...
namespace itk
{
  ...
  // Slice the neighborhood
		...
		this->ProcessObject::SetNumberOfRequiredOutputs(ImageDimension + 6); //8 = Ra + Rb + Rc + Vess + M + S + Vectors
			...
			//typename OutputImageType::Pointer magnitudeImage = OutputImageType::New();
			//this->ProcessObject::SetNthOutput(5, magnitudeImage.GetPointer());
...
// 		this->m_GradientMagnitudeFilter->SetInput(input);
// 		this->m_GradientMagnitudeFilter->SetNormalizeAcrossScale(true);
                               
               // const InputImageType:: spacing = this->GetInput()->GetSpacing();
                double w = 0;//two times voxel size
                
                for(int k=0; k<ImageDimension; k++)
                {
                    w *= this->GetInput()->GetSpacing()[k];
                }
		...
		int scaleLevel = 1;
                ...
                int scaleLevel1 = 1;
                for (n = 0; n < w; n++)
                    if (sigmaTemp < m_SigmaMax)
                {
                    sigmaTemp = this->ComputeSigmaValue( scaleLevel1 );
                    scaleLevel1++;
                }
...
// 			m_GradientMagnitudeFilter->Update();
                        ...
                        {
                             ////Then do the double thresholding upon the edge reponses
                            T = Tmax; 
                            ...
                            if (sigma < w)
                            {
                                if( i < n && n > 1)
                                {
                                    T = Tmin * vcl_exp(i * ((vcl_log10(Tmax/Tmin)) / vcl_log10(vnl_math::e)) / (n - 1)); 
                                    i++;
                                }
                                m_UpperThreshold = Tmin;
                                m_LowerThreshold = T;
                            }
                            cout<<"Sigma: "<<sigma<<" Tmin: "<<m_LowerThreshold<<" Tmax: "<<m_UpperThreshold<<endl;
...
// 		ImageRegionIterator<OutputImageType> mit(m_GradientMagnitudeFilter->GetOutput(),
// 			this->m_GradientMagnitudeFilter->GetOutput()->GetLargestPossibleRegion());
		...
		while(!bit.IsAtEnd())
		{
			...
			objNew = it.Value();
            //mNew = mit.Value(); //Replaced by threshold filter result
			...
			if( globalArray[0] < objNew )
			{
				...
				for(int i = ImageDimension+4, j = ImageDimension-1; j >= 0; j--, i--)
					globalArray[i] = vectorsNew[j];
									
				oit4.Set( sigma );
			}
                        //Magnitude unavailable for now
// 			if(globalArray[4] < mNew)
// 			{
// 				m = 1;
// 				globalArray[4] = mNew;
// 			}
    ...
    ::HysteresisThresholding()
{
    ...
    ::FollowEdge(InputIndexType index)
    {
        ...
        ImageRegionIterator<TOutputImage> bit(this->GetOutput(5), this->GetOutput(5)->GetLargestPossibleRegion());
        
        //assign iterator radius
        Size<ImageDimension> radius; 
        radius.Fill(1);
        
        NeighborhoodType oit(radius, m_HessianToMeasureFilter->GetVesselnessOutput(),
                this->m_HessianToMeasureFilter->GetVesselnessOutput()->GetLargestPossibleRegion());
